...
import (
	...
	"github.com/AnnaKhairetdinova/wether-service/internal/client/http/geocoding"
	"github.com/AnnaKhairetdinova/wether-service/internal/client/http/open_meteo"
...
func main() {
	...
	conn, err := pgx.Connect(ctx, "postgres://Anna:password@localhost:54321/weather")
	if err != nil {
		panic(err)
	}
...
func initJobs(ctx context.Context, scheduler gocron.Scheduler, conn *pgx.Conn) ([]gocron.Job, error) {
	...
	geocodindClient := geocoding.NewClient(httpClient)
	...
	j, err := scheduler.NewJob(
		...
		gocron.NewTask(
			func() {
				geoRes, err := geocodindClient.GetCoords(city)
				if err != nil {
					log.Println(err)
					return
				}
				...
				_, err = conn.Exec(
					...
					city, timestamp, openMetRes.Current.Temperature2m,
				)
				if err != nil {
					log.Println(err)
					return
